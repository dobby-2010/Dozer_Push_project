...
for eps_trial in eps_list:
    for min_sample_trial in min_sample_list:
        ...
        clustering = DBSCAN(eps=eps_trial, min_samples=min_sample_trial).fit(Y)
        ...
        if n_clusters_ > 0:
            ...
            if len(np.unique(labels)) > 1: #check is lebels if greater than 1 which is has to be. IF not then likely all zeros and not useful anyway
                sil_score_new = metrics.silhouette_score(Y, labels)
...
db = clustering = DBSCAN(eps=eps_best, min_samples=min_sample_best).fit(Y)  #use min samples = 4
        #storing the labels formed by the DBSCAN

data_1["cluster"] = db.labels_
fig = go.Figure()
fig = px.scatter(x=data_1.easting, y=data_1.elevation, color=data_1.cluster)
...
#We need the values either side of the points which have been removed.#lets say were looking for 3 points either side
    varaince = 2 #how many points either side
