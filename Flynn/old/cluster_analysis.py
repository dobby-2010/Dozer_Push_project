...
def data_dif(data_old, data_new):
    #Data_old first (original data) 
    # data_new second (new data)
    # function for importing newest scan data and oldest scan data 
    #will output the difference bwteen the cross sections
    data_1 = pd.read_csv(data_old)
    data_1.columns = ['easting', 'northing', 'elevation']
    data_2 = pd.read_csv(data_new)
    ...
    return(data_diff_full)
    
#Check if it works
data_new = 'CE_36_B03-202305030845.csv'
data_old = 'CE_36_B03-202305040800.csv'

#data_dif(c1, c2)



def locations_to_inspect(data_new, data_old, elevationchange):
    '''
    This functntion will find all the locations for which there is a elevation change of greater than specfied.
    scan
    
    Attributes: 
    scan1 = older scan (csv file)
    scan2 = newer scan (csv file)
    elevation change = specified amount of elevation change to be considered an anomaly
    
    '''
    norths_to_inspect = []
    data_diff_full = data_dif(data_new, data_old)
    norths_to_inspect = []
    for i in data_diff_full["northing"].unique():
        x = data_diff_full.loc[
            (data_diff_full["northing"] == i)
            & ((data_diff_full["elevation"] < -6) | ((data_diff_full["elevation"] > elevation))),
            :,
        ]
        if not x.empty:
            norths_to_inspect.append(i)
    return norths_to_inspect
